...
public class CommentResponse {
    ...
    public static CommentResponse of(Comment comment) {
        ...
        if (comment.getChildren() != null) {
            coCo = comment.getChildren().stream().map(coComment->CommentResponse.ofCoco(coComment)).collect(Collectors.toList());
        }
    ...
    // 자식 댓글
    public static CommentResponse ofCoco(Comment comment) {
        String deletedAt = null;
        if (comment.getDeletedAt() != null) deletedAt = comment.getDeletedAt().format(DateTimeFormatter.ofPattern("yy.MM.dd HH:mm:ss"));
        return CommentResponse.builder()
                .id(comment.getId())
